...
import 'package:cooking_app/modules/review/lesson_review/LessonReviewRepository.dart';
import 'package:cooking_app/utils/LogManager.dart';
import 'package:flutter/material.dart';
import 'package:rxdart/rxdart.dart';

class LessonReviewsBloc extends Bloc{
  static const String TAG = "LessonReviewsBloc";

  static const String GET_LESSONS_REVIEWS = "get_lessons_reviews";

  final _repository = LessonReviewRepository();
  ...
  LessonReviewsBloc(){
    event.stream.listen((event) {
      switch (event.eventType) {
        case GET_LESSONS_REVIEWS:
          _getLessonReviewsList(event.data);
          break;
      }
    });
  }

  _getLessonReviewsList(int lessonId) async{
    if (_pageNumber == 0) {
    ...
    var result = await _repository.getLessonReviews(data);
